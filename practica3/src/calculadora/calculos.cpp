 ...
 * Modificado por: Yago Torres Garcia (funciones establecidas en la práctica 3)
...
unsigned mcm(int a, int b){
    ...
    unsigned minimocomunmultiplo = (double(a)*b)/mcd(a, b);
...
bool esCapicua(int n){
    ...
    unsigned longitud = numCifras(n);
    int numinverso=0;
    int numeroinicial = n;

    for (unsigned i = 0; i < longitud; i++) {

        numinverso = n%10 + numinverso*10;
        n = n / 10;

    }

    if (numeroinicial == numinverso) {
